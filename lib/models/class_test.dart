...
class ClassTest {
  ...
  ClassTest.fromMap(Map<String, dynamic> map) {
    ...
    stud_attendence =
        map[ClassTestFieldNames.stud_attendence] ?? StringHandlers.NotAvailable;
    subject_id = map[ClassTestFieldNames.subject_id] ?? 0;
    subject_name =
        map[ClassTestFieldNames.subject_name] ?? StringHandlers.NotAvailable;
    test_date =
        map[ClassTestFieldNames.test_date] ?? StringHandlers.NotAvailable;
    test_desc =
        map[ClassTestFieldNames.test_desc] ?? StringHandlers.NotAvailable;
