...
class LibraryBookDetails {
  ...
  LibraryBookDetails.fromMap(Map<String, dynamic> map) {
    bAuthor = map[LibraryBookDetailsFieldnames.bAuthor] ??
        StringHandlers.NotAvailable;
    bEdition = map[LibraryBookDetailsFieldnames.bEdition] ?? 0;
    bPub =
        map[LibraryBookDetailsFieldnames.bPub] ?? StringHandlers.NotAvailable;
    bTitle =
        map[LibraryBookDetailsFieldnames.bTitle] ?? StringHandlers.NotAvailable;
  ...
  factory LibraryBookDetails.fromJson(Map<String, dynamic> parsedJson) {
    return LibraryBookDetails(
      bAuthor: parsedJson[LibraryBookDetailsFieldnames.bAuthor] ??
          StringHandlers.NotAvailable,
      bEdition: parsedJson[LibraryBookDetailsFieldnames.bEdition] ?? 0,
      bPub: parsedJson[LibraryBookDetailsFieldnames.bPub] ??
          StringHandlers.NotAvailable,
      bTitle: parsedJson[LibraryBookDetailsFieldnames.bTitle] ??
          StringHandlers.NotAvailable,
