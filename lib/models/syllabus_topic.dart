...
class SyllabusTopic {
  ...
  String details, topic_name, status;
  int no_of_sessions, topic_no, ent_no, subject_id;
  ...
  SyllabusTopic.fromMap(Map<dynamic, dynamic> map)
      : details =
            map[SyllabusTopicFieldNames.details] ?? StringHandlers.NotAvailable,
        topic_name = map[SyllabusTopicFieldNames.topic_name] ??
        ...
        status = map[SyllabusTopicFieldNames.status] ?? 'Pending',
  ...
  factory SyllabusTopic.fromJson(Map<String, dynamic> map) {
    return SyllabusTopic(
        ...
        topic_no: map[SyllabusTopicFieldNames.topic_no] ?? 0,
        status:
            map[SyllabusTopicFieldNames.status] ?? StringHandlers.NotAvailable,
        ent_no: map[SyllabusTopicFieldNames.ent_no] ?? 0,
        subject_id: map[SyllabusTopicFieldNames.subject_id] ?? 0);
  }
...
class SyllabusTopicUrls {
  ...
  static const String POST_UPDATE_SYLLABUS_STATUS =
      'Syllabus/UpdateTopicStatus';
}
