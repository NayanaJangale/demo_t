...
class SchoolCalender {
  ...
  SchoolCalender.fromJson(Map<String, dynamic> map) {
    ...
    from_date = map[SchoolCalenderFieldNames.from_date] != null
        ? DateTime.parse(map[SchoolCalenderFieldNames.from_date])
        : null;
    to_date = map[SchoolCalenderFieldNames.to_date] != null
        ? DateTime.parse(map[SchoolCalenderFieldNames.to_date])
        : null;

    event_name =
        map[SchoolCalenderFieldNames.event_name] ?? StringHandlers.NotAvailable;
    event_descr = map[SchoolCalenderFieldNames.event_descr] ??
        StringHandlers.NotAvailable;
    event_cat_id = map[SchoolCalenderFieldNames.event_cat_id] ?? 0;
    event_cat_name = map[SchoolCalenderFieldNames.event_cat_name] ??
        StringHandlers.NotAvailable;
  }
